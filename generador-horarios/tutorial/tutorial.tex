...
\lstdefinelanguage{CommonLisp}{
  morekeywords={defun,defvar,defmacro,defclass,let,let*,when,unless,
                ...
                def-op-mayor,def-op-menor,
                def-op-mayor-igual,def-op-menor-igual,
                generar-python},
  sensitive=false,
  morecomment=[l]{;;},
  morestring=[b]",
...
\midrule
\code{def-op-and e1 e2 \ldots} & Conjunción ($e_1 \wedge e_2 \wedge \cdots$) \\
\code{def-op-or  e1 e2 \ldots} & Disyunción ($e_1 \vee e_2 \vee \cdots$) \\
\code{def-op-not e}            & Negación ($\neg e$) \\
\code{def-op-igual a b}        & Igualdad ($a = b$) \\
\code{def-op-distinto a b}     & Desigualdad ($a \neq b$) \\
\code{def-op-mayor a b}        & Mayor estricto ($a > b$) \\
\code{def-op-menor a b}        & Menor estricto ($a < b$) \\
\code{def-op-mayor-igual a b}  & Mayor o igual ($a \geq b$) \\
\code{def-op-menor-igual a b}  & Menor o igual ($a \leq b$) \\
\bottomrule
\end{tabular}
\end{table}
...
(def-consulta profesor-en-tribunal (prof asig)
  :comprueba
  (def-op-or
    (def-op-igual (tutor      (tribunal asig)) prof)
...
\code{def-op-and e1 e2 ...} & Conjunción de expresiones \\
\code{def-op-or e1 e2 ...} & Disyunción de expresiones \\
\code{def-op-not e} & Negación \\
\code{def-op-igual a b} & Igualdad \\
\code{def-op-distinto a b} & Desigualdad \\
\code{def-op-mayor a b} & $a > b$ \\
\code{def-op-menor a b} & $a < b$ \\
